[Início](index.md) / **Visão Geral do Brasil**

---

# Visão Geral do Brasil

Esta página apresenta a consolidação macroterritorial do Brasil sob a ótica da divisão regional por **Regiões Geográficas Intermediárias** e **Regiões Geográficas Imediatas (Condados Funcionais)**, conforme estabelecido pelo Instituto Brasileiro de Geografia e Estatística (IBGE).

---

## Síntese Territorial Nacional

* **Unidades Federativas:** 27 (26 Estados + Distrito Federal)
* **Total de Municípios:** 5.570
* **Total de Regiões Intermediárias:** 133
* **Total de Condados Funcionais (RGIs):** 510
* **População Nacional (Censo IBGE):** ~203 milhões de habitantes
* **Área Territorial:** 8.510.417,7 km²

---

## Navegação por Grandes Regiões

Selecione um estado dentro das 5 grandes regiões para acessar o panorama regional e os condados correspondentes:

### 1. Região Sul

* [Paraná (PR)](brazil/pr/index.md)
* Santa Catarina (SC) — *Em breve*
* Rio Grande do Sul (RS) — *Em breve*

### 2. Região Sudeste

* Espírito Santo (ES) — *Em breve*
* Minas Gerais (MG) — *Em breve*
* Rio de Janeiro (RJ) — *Em breve*
* São Paulo (SP) — *Em breve*

### 3. Região Centro-Oeste

* Distrito Federal (DF) — *Em breve*
* Goiás (GO) — *Em breve*
* Mato Grosso (MT) — *Em breve*
* Mato Grosso do Sul (MS) — *Em breve*

### 4. Região Nordeste

* Alagoas (AL) — *Em breve*
* Bahia (BA) — *Em breve*
* Ceará (CE) — *Em breve*
* Maranhão (MA) — *Em breve*
* Paraíba (PB) — *Em breve*
* Pernambuco (PE) — *Em breve*
* Piauí (PI) — *Em breve*
* Rio Grande do Norte (RN) — *Em breve*
* Sergipe (SE) — *Em breve*

### 5. Região Norte

* Acre (AC) — *Em breve*
* Amapá (AP) — *Em breve*
* Amazonas (AM) — *Em breve*
* Pará (PA) — *Em breve*
* Rondônia (RO) — *Em breve*
* Roraima (RR) — *Em breve*
* Tocantins (TO) — *Em breve*

---

## Metodologia de Agregação Nacional

Assim como nas páginas estaduais, os dados agregados da visão nacional seguem os critérios:

1. **População e Área:** Soma direta de todos os municípios que compõem cada unidade geográfica.
2. **Densidade Populacional:** Razão simples entre a população acumulada e a área acumulada ($\text{hab/km}^2$).
3. **IDHM Consolidado:** Calculado através da **média ponderada populacional** dos municípios integrantes de cada condado ou região.

---

## Acesso aos Dados

Os arquivos de dados brutos podem ser consultados e baixados diretamente no repositório:

* [Diretório de Dados (data/)](https://github.com/NexaRift/br-counties/tree/main/data)